...
class BinaryStraightThroughScalarFunction(Function):
    ...
    def backward(ctx, grad_output):
        ...
        # s = input[0].size()
        # n = s[1]
        # m = input[0].norm(1, dim=1, keepdim=True).div(n).expand(s)
        # # print(m.shape, m)
        # m[input[0].lt(-1.0)] = 0
        # m[input[0].gt(1.0)] = 0
        # m = m.mul(grad_input)
        #
        # m_add = input[0].sign().mul(grad_input)
        # m_add = m_add.sum(1, keepdim=True).expand(s)
        # m_add = m_add.mul(input[0].sign()).div(n)
